...
public class PaymentController {
    ...
    public String listPayments(
        ...
        refType = (refType != null && !refType.trim().isEmpty()) ? refType.trim() : null;
        paymentStatus = (paymentStatus != null && !paymentStatus.trim().isEmpty()) ? paymentStatus.trim() : null;

        Page<Payment> paymentsPage;

        // ✅ SAFE ENUM CONVERSION
        Payment.RefType refTypeEnum = null;
        if (refType != null) {
            try {
                refTypeEnum = Payment.RefType.valueOf(refType.toUpperCase());
                System.out.println("✅ Converted refType to enum: " + refTypeEnum);
            } catch (IllegalArgumentException e) {
                System.out.println("⚠️ Invalid RefType provided: '" + refType + "'. Valid values: " + Arrays.toString(Payment.RefType.values()));
                refTypeEnum = null;
            }
        }
        ...
        if (paymentStatus != null) {
            try {
                statusEnum = Payment.PaymentStatus.valueOf(paymentStatus.toUpperCase());
                System.out.println("✅ Converted paymentStatus to enum: " + statusEnum);
            } catch (IllegalArgumentException e) {
                System.out.println("⚠️ Invalid PaymentStatus provided: '" + paymentStatus + "'. Valid values: " + Arrays.toString(Payment.PaymentStatus.values()));
                statusEnum = null;
            }
        }

        System.out.println("🔍 Final enum values - refTypeEnum: " + refTypeEnum + ", statusEnum: " + statusEnum);

        try {
            if (search != null || refTypeEnum != null || statusEnum != null) {
                System.out.println("🔍 Calling filterPayments with: search='" + search + "', refTypeEnum=" + refTypeEnum + ", statusEnum=" + statusEnum);
            ...
            System.err.println("❌ Error in repository call: " + e.getMessage());
            e.printStackTrace();
            // Fallback to empty page
            paymentsPage = Page.empty(pageable);
        }

        // Rest of your model attributes...
        ...
        model.addAttribute("currentSize", size); // ✅ Add this
        model.addAttribute("sort", sort);
        model.addAttribute("direction", direction);
        model.addAttribute("search", search);
        model.addAttribute("selectedRefType", refType);
        model.addAttribute("paymentStatus", paymentStatus); // ✅ Add this
