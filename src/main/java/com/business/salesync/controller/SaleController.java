...
public class SaleController {
    ...
    public String orders(
        ...
        if (dateRange != null && !dateRange.isEmpty()) {
            switch (dateRange) {
                ...
                case "week" -> {
                    ...
                    toDate = now.with(java.time.DayOfWeek.SUNDAY);
                }
                case "month" -> {
                ...
                case "year" -> {
                    ...
                    toDate = now.withDayOfYear(now.lengthOfYear());
                }
            }
        }

        // --- CONVERT paymentStatus String → Enum (if needed) ---
        SalesOrder.PaymentStatus statusEnum = null;
        if (paymentStatus != null && !paymentStatus.isBlank()) {
            try {
                statusEnum = SalesOrder.PaymentStatus.valueOf(paymentStatus.toUpperCase());
            } catch (IllegalArgumentException e) {
                System.err.println("Invalid payment status: " + paymentStatus);
            }
        }

        // --- DEBUG LOG ---
        System.out.printf("Filters => search=%s, payment=%s, enum=%s, dateRange=%s, from=%s, to=%s%n",
                search, paymentStatus, statusEnum, dateRange, fromDate, toDate);
        ...
        if (hasFilter) {
            orderPage = orderRepository.findWithFilters(search, statusEnum, dateRange, fromDate, toDate, pageInfo);
        } else {
        ...
        model.addAttribute("currentPaymentStatus", paymentStatus != null ? paymentStatus : "");
        model.addAttribute("currentDateRange", dateRange != null ? dateRange : "");
    ...
    public ResponseEntity<Map<String, Object>> getOrderDetails(@PathVariable Long id) {
        try {
            ...
            response.put("warrantyPeriod", order.getWarrantyPeriod());
            response.put("warrantyDescription", order.getWarrantyDescription());
            response.put("guaranteePeriod", order.getGuaranteePeriod());
            response.put("guaranteeDescription", order.getGuaranteeDescription());
    ...
    public String createUpdateOrder(@Valid SalesOrder order, HttpServletRequest request, Errors errors, Model model) {
     ...
     // parse the warranty/guarantee fields from request
        String warrantyPeriodStr = request.getParameter("warrantyPeriod");
        String warrantyDesc = request.getParameter("warrantyDescription");
        String guaranteePeriodStr = request.getParameter("guaranteePeriod");
        String guaranteeDesc = request.getParameter("guaranteeDescription");

        if(warrantyPeriodStr != null && !warrantyPeriodStr.isEmpty()) {
            order.setWarrantyPeriod(Integer.parseInt(warrantyPeriodStr));
        }
        order.setWarrantyDescription(warrantyDesc != null ? warrantyDesc : "");

        if(guaranteePeriodStr != null && !guaranteePeriodStr.isEmpty()) {
            order.setGuaranteePeriod(Integer.parseInt(guaranteePeriodStr));
        }
        order.setGuaranteeDescription(guaranteeDesc != null ? guaranteeDesc : "");
